namespace Zhiyun.Utilities.IO.Compression
{
    public class SingleZipContext : ZipContext
    {
        private List<(string, string)> Paths { get; } = [];

        public void Add(string path, string id)
        {
            if (File.Exists(path)) Paths.Add((path, id));
        ...
        public void Remove(IEnumerable<string> datas)
            => Paths.RemoveAll(s => datas.Contains(s.Item1));
        public void Clear()
            => Paths.Clear();
